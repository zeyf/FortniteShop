...
const ShopItemCard = ({price, image, id, rarity, name, BundleName, BundleStatus, setname}) => {
    ...
    const ShopLink = (category, handledName) => {
        if (BundleStatus) {
    ...
    return (
        <div className='s'>
            <div className='shopitemcard shopitemcard--primary' style={rarity} >
                <Link to={ShopLink(SetLinkByIDType(id), NameCharacterHandler(CheckBundleSetName(setname, name)))} className='shopitemcard__link' onClick={() => {
                    ...
                    <div className="shopiteminfo shopiteminfo--primary">
                        <p className='shopiteminfo__name'>{BundleName ? BundleName.toUpperCase() : name.toUpperCase()}</p>
